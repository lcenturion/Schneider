...
#define SSerialTxControl D2     //RS485 Direction control
...
void Modbus::begin(long u32speed) {
  ...
  //port->begin(u32speed, 0b00110000);
  port->begin(u32speed, SERIAL_8N1);
  //port->begin(u32speed, FLOW_NONE | WL_8 | PAR_NONE | SB_1);
  //port->begin(u32speed);
...
int8_t Modbus::poll() {
  ...
  uint8_t u8current = port->available();
        ...
        if(u8current!=0)
        {
            //Serial.print("POLL port->avilable()=");
  ...
  uint8_t u8exception = validateAnswer();
      #ifdef LOGGING
      Serial.print("Validate Message Exception=");
      Serial.println(u8exception, HEX);
    #endif
  if (u8exception != 0) {
    u8state = COM_IDLE;
    #ifdef LOGGING
  ...
  // process answer
    #ifdef LOGGING
      Serial.println("au8Buffer[FUNC]=");
      Serial.print(au8Buffer[ FUNC ], HEX);
    #endif
...
int8_t Modbus::poll( uint16_t *regs, uint8_t u8size ) {
  ...
  u8lastRec = 0;
    #ifdef LOGGING
      Serial.println("GET RX BUFFER");
    #endif
  int8_t i8state = getRxBuffer();
  u8lastError = i8state;
    #ifdef LOGGING
      Serial.println("i8State=");
      Serial.print(i8state, HEX);
    #endif
  if (i8state < 7) return i8state;
    #ifdef LOGGING
      Serial.println("Check Slave Address ID=");
      Serial.print(au8Buffer[ ID ], HEX);
    #endif
  // check slave id
  ...
  uint8_t u8exception = validateRequest();
    #ifdef LOGGING
      Serial.println("Validate Message Exception=");
      Serial.print(u8exception, HEX);
    #endif
  if (u8exception > 0) {
...
int8_t Modbus::getRxBuffer() {
  ...
  if(port->available())
  {
      uint8_t first = port->read();
      if(first != 0)
      {
        au8Buffer[ u8BufferSize++ ] = first;
      }
  }
